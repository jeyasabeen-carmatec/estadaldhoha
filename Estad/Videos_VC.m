...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    static NSString *identifier = @"video_CELL";
    
    video_VC_cell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:indexPath];
    
     NSDictionary *Video;
     @try {
     Video = [[json_RESULT objectAtIndex:indexPath.row] valueForKey:@"Video"];
     } @catch (NSException *exception) {
     NSLog(@"Exception1 = %@",exception);
     }
    
    NSArray *temp = [[Video valueForKey:@"embed_code"] componentsSeparatedByString:@"/"];
    NSString *video_ID = [NSString stringWithFormat:@"%@",[temp objectAtIndex:[temp count] - 1]];
    
    [cell.playerView loadWithVideoId:video_ID];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if (tableView == _list_DATA)
    {
        ...
        if ([model_Name isEqualToString:@"News"])
        {
            ...
            controller.get_Video_titl = @"Video";
...
-(void) Decide_API
{
    str_URL = [NSString stringWithFormat:@"%@videolist/20/%@",MAIN_URL,[self getUTCFormateDate:[NSDate date]]];
    [self get_DATA];
}
-(void) get_DATA
{
