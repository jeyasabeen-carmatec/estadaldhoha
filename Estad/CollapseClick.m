...
-(void)reloadCollapseClick {
    ...
    for (int xx = 0; xx < [CollapseClickDelegate numberOfCellsForCollapseClick]; xx++) {
        ...
        cell.ContentView.frame = CGRectMake(0, kCCHeaderHeight + kCCPad, cell.ContentView.frame.size.width, cell.ContentView.frame.size.height);
...
//        NSLog(@"Cell view frame = %@",NSStringFromCGRect(cell.frame));
...
-(void)didSelectCollapseClickButton:(UIButton *)titleButton {
    ...
    if (i >= 2 && i <= 4)
    {
        ...
        if ([[self.isClickedArray objectAtIndex:titleButton.tag] boolValue] == YES) {
            [self closeCollapseClickCellAtIndex:i animated:YES];
        }
        // Cell is CLOSED -> OPEN
        else {
            [self openCollapseClickCellAtIndex:i animated:YES];
            isOpen = YES;
        }
    }
    ...
    if ([(id)CollapseClickDelegate respondsToSelector:@selector(didClickCollapseClickCellAtIndex:isNowOpen:)]) {
        [CollapseClickDelegate didClickCollapseClickCellAtIndex:i isNowOpen:isOpen];
    }
}
...
//            CGAffineTransform transform = CGAffineTransformMakeRotation(M_PI/2);
            CGAffineTransform transform = CGAffineTransformRotate(CGAffineTransformMakeScale(-1, 1), M_PI);
