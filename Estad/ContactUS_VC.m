...
-(void) setup_VIEW
{
    ...
    label.text = NSLocalizedString(@"اتصل بنا", @"");
    [label sizeToFit];
    self.navigationItem.titleView = label;
       ...
       /* if (indexPath.row == 0)
        {
            ...
            if (cell == nil)
            {
                NSArray *nib = [[NSBundle mainBundle] loadNibNamed:@"Cell_CONTACT_US_F" owner:self options:nil];
                cell = [nib objectAtIndex:0];
            }
//            cell.lbl_content.text = [list_MEDIA objectAtIndex:indexPath.row];
            return cell;
        ...
        {*/
           ...
           if (indexPath.row == 0) {
                ...
                cell.lbl_title.text = @"صندوق البريد";
                cell.lbl_contents.text = @"24598";
                cell.img_logo.image = [UIImage imageNamed:@"mailbox"];
            }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if (tableView == _list_DATA)
    {
        ...
        if ([model_Name isEqualToString:@"News"])
        {
            ...
            controller.get_ontact_US = @"ContactUS";
